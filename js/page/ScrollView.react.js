...
var Rotator = React.createClass({
	...
	render: function() {
		...
		var style = STYLE_CLASS;
		if (this.state.toRotate) {
			style += ' rotate';
		}

		return (
			<div ref='rotator' className={style}>
				<img />
			</div>
		);
	}
...
var Loader = React.createClass({
	...
	componentDidMount: function() {
		this._loaderNode = ReactDom.findDOMNode(this.refs.scrollLoader);
	},
...
var Refresh = React.createClass({
	...
	componentDidMount: function() {
		this._refreshNode = ReactDom.findDOMNode(this.refs.scrollRefresh);
		this._refreshTextNode = ReactDom.findDOMNode(this.refs.refreshText);
	},
...
var ScrollView = React.createClass({
	...
	propTypes: {
		pullToRefresh: React.PropTypes.bool,
		pullToLoad: React.PropTypes.bool,
		hasMore: React.PropTypes.bool,
		onStartLoading: React.PropTypes.func,
		onStartRefresh: React.PropTypes.func,
		onScrollUp: React.PropTypes.func,
	...
	componentDidMount: function() {
		...
		this._scrollViewNode = ReactDom.findDOMNode(this.refs.scrollView);
		this._scrollContentNode = ReactDom.findDOMNode(this.refs.scrollContent);
		this._scrollContentNode.addEventListener('webkitTransitionEnd', this._transitionEnd);

		// 计算尺寸
		this._calcSize();
		// console.log(-this._refreshHeight);
	...
	setWrapperHeight: function(wrapperHeight) {
	...
	render: function() {
		...
		return (
			<div ref='scrollView' className='b-scroll-view'
				 style={this.props.style}
				onTouchCancel={this._scrollEnd} onTouchEnd={this._scrollEnd} onTouchMove={this._scrollMove} onTouchStart={this._scrollStart} >
				{refreshComp}
				<div ref='scrollContent'className='b-scroll-content'>
					<div className='b-content' ref='bContent'>
						{this.props.children}
					</div>
					{loaderComp}
				</div>
	...
	_calcSize: function () {
        ...
        this._wrapperHeight = this._scrollViewNode.offsetHeight;
        this._wrapperWidth = this._scrollViewNode.clientWidth;
        
        this._scrollerWidth = this._scrollContentNode.scrollWidth;
        this._scrollerHeight = this._scrollContentNode.offsetHeight + this._loaderHeight;
        // console.log('##### content height=' + React.findDOMNode(this.refs.bContent).offsetHeight);
        // console.log('##### height=' + this._scrollContentNode.offsetHeight);
        // console.log(this._scrollContentNode);

        if (this._wrapperHeight == 0) {
        	this._wrapperHeight = this._scrollerHeight;
        }
    ...
    _getResetPositionLoad: function() {
    	...
    	return {
    		x: this._x,
    		y: -this._refreshHeight + -this._scrollerHeight + this._wrapperHeight,
    		time: PULL_TO_SNAP_TIME
    	}
    },
	...
	_resetPosition: function(time, forceReset) {
		...
		// 如果正在加载
			...
			// console.log('Is loading, y = ' + customPos.y)
		}
